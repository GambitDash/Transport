...
Fail:
	...
	pStream->Close();
	*retsError = strPatternSubst(LITERAL("Line(%d): %s"), Ctx.iLine, Ctx.sError.GetPointer());
	return error;
	}
...
//	Parses <?XML prologue
	...
	//	Expect open processor instruction
				...
				switch (chChar)
					...
					case ';':
						...
						if (!bFound)
							{
							pCtx->iToken = tkError;
							pCtx->sError = strPatternSubst(CONSTLIT("Invalid entity: %s"), sEntity.GetPointer());
							bDone = true;
							break;
							}
	...
	if (*pPos == '#')
		...
		if (*pPos == 'x' || *pPos == 'X')
			...
			*pPos++;
			...
			{
			char chChar = (char)strParseInt(pPos, 0x20, NULL, NULL);
			return CString(&chChar, 1);
			}
		}
