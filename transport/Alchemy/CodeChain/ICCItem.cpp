...
//	Returns an error when item is not a symbol table
	...
	{
	return pCC->CreateError(LITERAL("Not a symbol table."), NULL);
	}
...
//	Stream the item
	...
	error = pStream->Write((char *)&dwFlags, sizeof(dwFlags), NULL);
	if (error)
...
//	Unstream the item
	...
	error = pStream->Read((char *)&dwFlags, sizeof(dwFlags), NULL);
	if (error)
		return pCC->CreateSystemError(error);
