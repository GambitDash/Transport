...
//	Initializes the object. Clean up is done in the destructor
	...
	for (i = 0; i < DEFPRIMITIVES_COUNT; i++)
		error = RegisterPrimitive(&g_DefPrimitives[i]);
		if (error)
			return error;
...
//	pData: Item that caused error.
	...
	if (pData)
		...
		sErrorLine = strPatternSubst(LITERAL("%s %s"), sError.GetASCIIZPointer(), sArg.GetASCIIZPointer());
		}
	else
		sErrorLine = sError;
		...
		if (pResult->IsError())
			...
			if (pError)
				...
				if (!sError.IsBlank())
					...
					if (*pPos != '#')
						{
						sError.Append(strPatternSubst(CONSTLIT(" ### %s ###"), pItem->Print(this).GetASCIIZPointer()));
...
//	Returns the binding for this item
	...
	/* XXX Disabled due to pStart ending up as NULL while iFrame was not 0. */
	/* XXX Subsequent tests show that enabling this works fine but has horrible
	 * performance.  It might be worth more research, but it might not as well.
	 */
	if (0 && pItem->GetBinding(&iFrame, &iOffset))
		{
		while (iFrame > 0)
			{
...
//	Save the item to an open stream
	...
	error = pStream->Write((char *)&dwClass, sizeof(dwClass), NULL);
	if (error)
...
//	Load the item from an open stream
	...
	error = pStream->Read((char *)&dwClass, sizeof(dwClass), NULL);
	if (error)
		return CreateSystemError(error);
