...
CString CCVector::Print (CCodeChain *pCC)
...
//	Sets the size of the vector, preserving any previous data
	...
	if (iNewSize > 0)
		...
		if (m_pData)
			...
			iInitialized = std::min(m_iCount, iNewSize);
...
//	Streams the vector to a stream
	...
	if (m_pData)
		{
		error = pStream->Write((char *)m_pData, m_iCount * sizeof(DWORD), NULL);
		if (error)
			return pCC->CreateSystemError(error);
		}
...
//	Reads the vector from a stream
	...
	if (m_pData)
		{
		error = pStream->Read((char *)m_pData, iCount * sizeof(DWORD), NULL);
		if (error)
			return pCC->CreateSystemError(error);
		}
