...
//	Fly, fight, die
	...
	switch (m_State)
		...
		case stateWaitingForThreat:
			...
			ImplementFormationManeuver(vPos, NullVector, VectorToPolar(m_vThreatPotential));
...
//	Compute the point at which to rally to attack pTarget based on an initial angle
	...
	if (m_pAISettings->iCombatStyle == aicombatStandOff)
		...
		if (m_iBestWeapon != devNone)
			rDistance = m_pBestWeapon->GetMaxEffectiveRange(m_pShip, pTarget);
		else
...
//	Attack objective and all enemies
	...
	ImplementAttackTarget(m_pObjective);
...
//	Attack objective from rally point
	...
	ImplementFormationManeuver(m_vRallyPoint, NullVector, m_iRallyFacing);
...
//	Charge towards objective
	...
	{
	ImplementFormationManeuver(m_pObjective->GetPos(), NullVector, m_iRallyFacing);
...
//	Handle the case where another object is destroyed
	...
	if (Ctx.pObj->GetCategory() == CSpaceObject::catShip)
		...
		if (RemoveAssetObj(Ctx.pObj))
			...
			if (Ctx.pDestroyer 
					&& (pTarget = FindTarget(Ctx.pDestroyer)) != NULL)
...
//	DWORD		flags
	...
	m_pShip->GetSystem()->WriteObjRefToStream(m_pObjective, pStream);
	...
	for (i = 0; i < m_iAssetCount; i++)
		{
		m_pShip->GetSystem()->WriteObjRefToStream(m_pAssets[i].pAsset, pStream);
		pStream->Write((char *)&m_pAssets[i].iFormationPos, sizeof(DWORD));
		m_pShip->GetSystem()->WriteObjRefToStream(m_pAssets[i].pTarget, pStream);
		}

	pStream->Write((char *)&m_iTargetCount, sizeof(DWORD));
	for (i = 0; i < m_iTargetCount; i++)
		{
		m_pShip->GetSystem()->WriteObjRefToStream(m_pTargets[i].pTarget, pStream);
...
//	Updates the targets array and the potential threat vector
	...
	for (i = 0; i < m_pShip->GetSystem()->GetObjectCount(); i++)
		...
		if (pObj  == NULL || pObj == m_pObjective || pObj == m_pShip)
			NULL;
