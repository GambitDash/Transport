...
//	Returns debug crash info
	...
	sResult.Append(strPatternSubst(CONSTLIT("m_pLeader: %s\r\n"), CSpaceObject::DebugDescribe(m_pLeader).GetASCIIZPointer()));
...
CSpaceObject *CFleetShipAI::GetTarget (void)
...
//	Fire on the nearest target
	...
	{
	if (m_pShip->IsDestinyTime(20))
		m_pTarget = m_pShip->GetNearestEnemy();

	if (m_pTarget)
		{
		bool bOutOfRange;

		ImplementFireOnTarget(m_pTarget, &bOutOfRange);
		if (bOutOfRange)
			m_pTarget = NULL;
		}
	}
...
//	Stay in formation
	...
	ImplementFormationManeuver(vDest, vDestVel, m_pLeader->GetRotation(), retbInFormation);
	}
...
//	DWORD		m_vVector
	...
	m_pShip->GetSystem()->WriteObjRefToStream(m_pLeader, pStream);
