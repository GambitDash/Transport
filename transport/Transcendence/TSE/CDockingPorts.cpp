...
//	Move all docked objects to follow the owner
	...
	for (int i = 0; i < m_iPortCount; i++)
		if (m_pPort[i].iStatus == psInUse)
			m_pPort[i].pObj->Place(vPos + m_pPort[i].vPos, vVel);
	}
...
void CDockingPorts::OnOwnerChanged (CSpaceObject *pOwner)
...
//	OnOwnerChanged 
	...
	for (int i = 0; i < m_iPortCount; i++)
		if (m_pPort[i].pObj)
			m_pPort[i].pObj->OnDockedObjChanged();
	}
...
//	WriteToStream 
	...
	for (int i = 0; i < m_iPortCount; i++)
		...
		pOwner->GetSystem()->WriteObjRefToStream(m_pPort[i].pObj, pStream);
