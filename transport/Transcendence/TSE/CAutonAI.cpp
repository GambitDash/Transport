...
//	Initiate behavior state based on orders
	...
	switch (GetCurrentOrder())
		...
		case IShipController::orderEscort:
			...
			if (m_pAISettings->dwFlags & aiflagNonCombatant)
...
CSpaceObject *CAutonAI::GetOrderGiver (void)
...
CSpaceObject *CAutonAI::GetTarget (void)
	...
	switch (iMessage)
		...
		case msgAttack:
			...
			if (GetCurrentOrder() == IShipController::orderEscort
					&& !(m_pAISettings->dwFlags & aiflagNonCombatant))
				{
...
void CAutonAI::OnPlayerObj (CSpaceObject *pPlayer)
...
//	Player has entered the system
	...
	{
	if (GetCurrentOrder() == IShipController::orderWaitForPlayer)
		{
		CancelCurrentOrder();
