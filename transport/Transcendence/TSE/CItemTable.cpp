...
//	Creates a new generator
	...
	if (strEquals(pDesc->GetTag(), ITEM_TAG))
		...
		{
		Ctx.sError = strPatternSubst(CONSTLIT("Unknown item generator: %s"), pDesc->GetTag().GetASCIIZPointer());
		return ERR_FAIL;
		}
...
//	Create from XML descriptor
	...
	if (pElement)
		...
		if (error = IItemGenerator::CreateFromXML(Ctx, pElement, &m_pGenerator))
			{
			Ctx.sError = strPatternSubst(CONSTLIT("ItemTable (%x): %s"), GetUNID(), Ctx.sError.GetASCIIZPointer());
			return error;
			}
		}
...
void CSingleItem::AddItems (CItemListManipulator &ItemList)
...
void CLookup::AddItems (CItemListManipulator &ItemList)
...
void CTableOfGenerators::AddItems (CItemListManipulator &ItemList)
...
//	Add items
	...
	for (i = 0; i < m_iCount; i++)
		...
		if (iRoll <= 0)
			...
			for (j = 0; j < iCount; j++)
				m_Table[i].pItem->AddItems(ItemList);
			break;
			}
		}
	}
...
//	Load from XML
	...
	if (m_iCount > 0)
		...
		memset(m_Table, 0, sizeof(SEntry) * m_iCount);
		...
		if (iTotal != 100)
			{
			Ctx.sError = CONSTLIT("Item table probabilities do not add up to 100%");
			return ERR_FAIL;
			}
		}
	else
...
void CGroupOfGenerators::AddItems (CItemListManipulator &ItemList)
...
void CRandomItems::AddItems (CItemListManipulator &ItemList)
...
//	Add items
	...
	for (i = 0; i < m_iCount; i++)
		...
		if (iRoll <= 0)
			...
			if (bAllAtOnce)
				ItemList.AddItem(CItem(m_Table[i].pType, iCount));
...
//	Load from XML
	...
	if (sCriteria.IsBlank())
		sCriteria = strPatternSubst(CONSTLIT("%s %s"), pDesc->GetAttribute(CATEGORIES_ATTRIB).GetASCIIZPointer(), pDesc->GetAttribute(MODIFIERS_ATTRIB).GetASCIIZPointer());
...
//	Resolve references
	...
	if (m_iCount == 0)
		{
		Ctx.sError = CONSTLIT("No items for random item table");
		return ERR_FAIL;
		}
...
//	Enhances the given item
	...
	if (m_pCode)
		...
		ICCItem *pOldValue = CC.LookupGlobal(STR_G_ITEM, &g_pUniverse);
		...
		ICCItem *pResult = CC.TopLevel(m_pCode, &g_pUniverse);
		...
		if (pResult->IsError())
			{
			CString sError = strPatternSubst(CONSTLIT("Generate Enhancement: %s"), pResult->GetStringValue().GetASCIIZPointer());
		...
		pResult->Discard(&CC);
		CC.DefineGlobal(STR_G_ITEM, pOldValue);
		pOldValue->Discard(&CC);
