...
void CTileMapSection::ReadFromStream (int iCount, IReadStream *pStream)
	{
	pStream->Read((char *)m_pMap, iCount * sizeof(CTile));
	}

void CTileMapSection::WriteToStream (int iCount, IWriteStream *pStream) const
	{
	pStream->Write((char *)m_pMap, iCount * sizeof(CTile));
	}
...
//	CTileMapSection recursion
	...
	while (dwOffset != 0xffffffff)
		...
		if (iDenom == 1)
			...
			pSection->ReadFromStream(iCount, pStream);
