	...
	switch (m_State)
		...
		case stateOnEscortCourse:
			...
			if (CalcFlockingFormation(m_pBase, MAX_FLOCK_DIST, SEPARATION_RANGE, &vFlockPos, &vFlockVel, &iFlockFacing))
				{
				ImplementFormationManeuver(vFlockPos, vFlockVel, iFlockFacing);
				}
			else
				{
				...
				ImplementCloseOnTarget(m_pBase, vTarget, rTargetDist2);
		...
		case stateOnPatrolOrbit:
			...
			if (CalcFlockingFormation(NULL, MAX_FLOCK_DIST, SEPARATION_RANGE, &vFlockPos, &vFlockVel, &iFlockFacing))
				...
				{
				ImplementOrbit(m_pBase, LIGHT_SECOND * GetCurrentOrderData());
				ImplementAttackNearestTarget(m_rMaxWeaponRange, &m_pTarget);
				ImplementFireOnTargetsOfOpportunity();
				}
...
//	Returns debug crash info
	...
	sResult.Append(CONSTLIT("CZoanthropeAI\r\n"));
...
CSpaceObject *CZoanthropeAI::GetOrderGiver (void)

//	GetOrderGiver
//
//	Returns the object that gives the orders
...
CSpaceObject *CZoanthropeAI::GetTarget (void)
...
//	Move and fire
	...
	int iFireDir;
	ImplementFireWeaponOnTarget(-1, -1, pTarget, vTarget, rTargetDist2, &iFireDir);
