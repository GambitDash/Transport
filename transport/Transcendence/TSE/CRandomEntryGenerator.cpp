...
//	Load a group element
	...
	if (pTable->m_iItemCount > 0)
		...
		//	Allocate a temporary weight table (in case we need to compute
		//	probabilities from weights).

		bool bComputePercentages = false;
		int *pWeightTable = new int[pTable->m_iItemCount];
		...
		for (i = 0; i < pTable->m_iItemCount; i++)
			...
			//	If this item has a weight then we need to compute percentages
			//	for the whole table

			pWeightTable[i] = pItem->GetAttributeInteger(WEIGHT_ATTRIB);
			if (pWeightTable[i])
				bComputePercentages = true;
			}

		//	If we need to compute percentages, do it now

		if (bComputePercentages)
			{
			ComputePercentages(pTable->m_iItemCount, pWeightTable);
			for (i = 0; i < pTable->m_iItemCount; i++)
				pTable->m_pItems[i]->SetPercent(pWeightTable[i]);
			}
	...
	int iTotal = 0;
	for (i = 0; i < pTable->m_iItemCount; i++)
		iTotal += pTable->m_pItems[i]->GetPercent();
	if (iTotal != 100)
		{
		ASSERT(false);
		return ERR_FAIL;
