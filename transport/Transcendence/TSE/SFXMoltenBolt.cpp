...
//	Creates a bolt-shaped polygon
	...
	XForm Trans(xformScale, ((Metric)iLength)/100.0, ((Metric)iWidth)/34.0);
	Trans = Trans * XForm(xformRotate, iAngle);

	//	Generate the points

	for (DWORD i = 0; i < SHAPE_COUNT; i++)
		{
		Metric x, y;
...
//	Paint effect
	...
	COLORREF wColor = CG16bitImage::BlendPixel(Ctx.wSpaceColor, m_wSecondaryColor, 200);
