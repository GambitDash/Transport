...
void CItemListManipulator::GenerateViewMap (const CItemCriteria *pFilter)
...
//	Generates a mapping from index to an index into the item list
	...
	for (int i = 0; i < m_ItemList.GetCount(); i++)
		...
		if (pFilter == NULL || ThisItem.MatchesCriteria(*pFilter))
...
void CItemListManipulator::Refresh (const CItem &Item)
...
void CItemListManipulator::SetChargesAtCursor (int iCharges)
...
void CItemListManipulator::SetDamagedAtCursor (bool bDamaged)
...
void CItemListManipulator::SetDataAtCursor (const CString &sAttrib, const CString &sData)
...
void CItemListManipulator::SetModsAtCursor (DWORD dwMods)

//	SetModAtCursor
//
//	Set modification at cursor
