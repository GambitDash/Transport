...
//	Create a new particle effect in the system
	...
	pType->m_fLifespan = false;
	pType->m_fRegenerate = false;
	pType->iRegenerationTimer = 0;
	pType->iLifespan = 0;
	...
	if (pDesc->FindAttribute(DAMAGE_ATTRIB, &sDamage))
		{
		if (error = pType->Damage.LoadFromXML(sDamage))
			return error;

		pType->m_fDamage = true;
		}
	else
	...
	pType->wColor = CGImage::RGBColor(0, 255, 0);
...
//	Paint the effect
	...
	while (pGroup)
		...
		switch (pType->iPaintStyle)
			...
			default:
				{
				COLORREF wColor = CGImage::RGBColor(0, 255, 0);
...
//	Vector		particle: vVel
	...
	while (dwNext != 0xffffffff)
		...
		pGroup->pType->wColor = (COLORREF)dwLoad;
		...
		pGroup->pType->Damage.ReadFromStream(Ctx);
		...
		pGroup->pType->m_fDamage =		((dwLoad & 0x00000020) ? true : false);
...
//	Paint particles as flame
	...
	while (pParticle < pEnd)
		...
		if (pParticle->IsValid())
			...
			if (iColor <= 50)
				wColor = CG16bitImage::FadeColor(CGImage::RGBColor(255, 255, 255),
						CGImage::RGBColor(250, 210, 10), 2 * iColor);
			else if (iColor <= 150)
				wColor = CG16bitImage::FadeColor(CGImage::RGBColor(250, 210, 10),
						CGImage::RGBColor(255, 20, 0), iColor - 50);
			else
				wColor = CGImage::RGBColor(255, 20, 0);
...
//	Paint particles as smoke
	...
	while (pParticle < pEnd)
		...
		if (pParticle->IsValid())
			...
			COLORREF wColor = CGImage::RGBColor(iColor, iColor, iColor);
...
bool CParticleEffect::PointInObject (CVector vPos)
...
//	Vector		particle: vVel
	...
	while (pGroup)
		...
		pType->Damage.WriteToStream(pStream);
		...
		dwSave |= (pType->m_fDamage ?		0x00000020 : 0);
		pStream->Write((char *)&dwSave, sizeof(DWORD));
