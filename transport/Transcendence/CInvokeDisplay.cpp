...
//	Update the buffer
	...
	if (iDeity == deityDomina)
		{
		rgbColor = CGImage::RGBColor(0, 160, 221);
		wColor = CGImage::RGBColor(0, 160, 221);
		wFadeColor = CGImage::RGBColor(0, 80, 110);
		sTitle = CONSTLIT("Invoke powers of Domina");
		}
	else
		{
		rgbColor = CGImage::RGBColor(185, 0, 0);
		wColor = CGImage::RGBColor(185, 0, 0);
		wFadeColor = CGImage::RGBColor(92, 0, 0);
		sTitle = CONSTLIT("Invoke powers of Oracus");
		}

	//	Clear

	m_Buffer.Fill(0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT, CGImage::RGBColor(0, 0, 0));
