...
//	Paints the device at the given location
	...
	int iLevel = pDevice->GetCounter(&iType);

	CString sTitle;
   COLORREF wLevelColor;
	COLORREF wBackColor;
...
//	Updates buffer
	...
	for (i = 0; i < pShip->GetDeviceCount(); i++)
		...
		if (!pDevice->IsEmpty() && pDevice->GetCounter())
			iCount++;
		}
	...
	if (m_Buffer.IsEmpty() || cxWidth != m_Buffer.GetWidth())
		{
