...
//	Paints the status and ammo for a device
	...
	if (pDevice)
		...
		CString sBonus;
		if (pDevice->GetMods())
			sBonus = pDevice->GetMods().GetEnhancedDesc(pShip->GetNamedDeviceItem(iDev), pShip);
		else if (pDevice->GetBonus() > 0)
			sBonus = strPatternSubst(CONSTLIT("+%d"), pDevice->GetBonus());
		else if (pDevice->GetBonus() < 0)
			sBonus = strPatternSubst(CONSTLIT("%d"), pDevice->GetBonus());
