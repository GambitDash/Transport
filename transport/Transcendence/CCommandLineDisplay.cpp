...
void CCommandLineDisplay::AppendOutput (const CString &sLine, COLORREF wColor)
...
COLORREF CCommandLineDisplay::GetOutputColor (int iLine)
...
void CCommandLineDisplay::Output (const CString &sOutput, COLORREF wColor)
...
//	Output the given string
	...
	int iStart = 0;
	while (iStart < sOutput.GetLength())
		{
		CString sLine = strSubString(sOutput, iStart, iCols);
		AppendOutput(sLine, wColor);
		iStart += sLine.GetLength();
		}
	}
...
//	Update the buffer
	...
	COLORREF wColor = CG16bitImage::RGBValue(0, 160, 221);
	COLORREF wFadeColor = CG16bitImage::RGBValue(0, 80, 110);
