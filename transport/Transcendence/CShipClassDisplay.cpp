...
const COLORREF RGB_BACKGROUND =					CG16bitImage::RGBValue(0, 0, 0);
const COLORREF RGB_CLASS_NAME =					CG16bitImage::RGBValue(128, 128, 128);
const COLORREF RGB_DESCRIPTION =				CG16bitImage::RGBValue(128, 128, 128);
const COLORREF RGB_ITEM_TITLE =					CG16bitImage::RGBValue(255, 255, 255);
const COLORREF RGB_ITEM_DESC =					CG16bitImage::RGBValue(128, 128, 128);
const COLORREF RGB_ITEM_NUMBER =				CG16bitImage::RGBValue(200, 200, 200);
...
ALERROR CShipClassDisplay::Init (CTranscendenceWnd *pTrans, const RECT &rcRect, bool bShowDebugShips)
...
//	Must be called to initialize
	...
	InitPlayerShipList(bShowDebugShips);
	InitShipData();
	...
	error = m_Images.CreateFromBitmap(hDIB, NULL);
	::DeleteObject(hDIB);
	if (error)
		return error;
...
void CShipClassDisplay::InitPlayerShipList (bool bShowDebugShips)
...
//	Initializes data for ship class
	...
	const SPlayerSettings *pPlayerSettings = pClass->GetPlayerSettings();
	...
	if (pPlayerSettings->m_dwLargeImage)
		m_pShipImage = g_pUniverse->GetLibraryBitmap(pPlayerSettings->m_dwLargeImage);
	...
	if (iBestWeapon != -1)
		...
		pData->sDesc = pItem->GetReference();
	...
	if (pClass->GetHullSectionCount() > 0)
		...
		pData->sHeading = strPatternSubst(CONSTLIT("%s (x%d)"), 
				(bSame ? pArmor->GetShortName() : CONSTLIT("armor segments")).GetASCIIZPointer(), 
		...
		pData->sDesc = strPatternSubst(CONSTLIT("%s; optional upgrade up to %s ton segments"),
				(bSame ? pArmor->GetItemType()->GetReference() : CONSTLIT("segments of different strengths")).GetASCIIZPointer(),
				sMaxArmor.GetASCIIZPointer());
	...
	if (pReactorDesc)
		...
		if (pReactor)
			...
			pData->sHeading = strPatternSubst(CONSTLIT("%s reactor"), pClass->GetShortName().GetPointer());
	...
	if (pCargoExtension)
		pData->sHeading = strPatternSubst(CONSTLIT("ton %s"), pCargoExtension->GetItemType()->GetNounPhrase(nounActual).GetASCIIZPointer());
	else
...
//	Paints a data box
	...
	if (DataBox.dwFlags & FLAG_ALIGN_LEFT)
		...
		if (!DataBox.sNumber.IsBlank())
			...
			rcRect.top += m_pFonts->LargeBold.GetHeight() - pHeaderFont->GetHeight();
			}
